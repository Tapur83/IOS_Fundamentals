...
func progressUpdate (steps: Int, goal: Int) -> String {
    ...
    } else if steps < goal || steps > goal * 90 / 100{
        return "You're almost there"
    } else{
        return "Congratulations!! You beat your goal!"
    }
        }

let result = progressUpdate(steps: 9000, goal: 10000)
...
func pacing(currentDistance: Double, totalDistance: Double, currentTime: Double, goalTime: Double) -> String {
    let speedNeeded = (totalDistance / goalTime)
    let speedOfPerson = (currentDistance / currentTime)
    
    
    if speedOfPerson < speedNeeded {
        return "You've got to push it just a bit harder!"
    } else {
        return "You going to do it. if in the same speed"
    }
}

let result1 = pacing(currentDistance: 3.2, totalDistance: 10.0, currentTime: 11.0, goalTime: 20.0)
let result2 = pacing(currentDistance: 5.0, totalDistance: 10.0, currentTime: 10.0, goalTime: 20.0)
